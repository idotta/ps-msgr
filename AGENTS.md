...
- The GitHub integration appends a "Generated by Claude Code" footer to every
  pull request it creates, even when the given description has none. Right
  after creating a pull request, edit its description to remove the footer,
  then read it back to confirm it is gone. Edits do not re-add it.
